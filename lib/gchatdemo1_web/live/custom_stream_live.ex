defmodule Gchatdemo1Web.CustomStreamLive do
  ...
  def handle_event("convert_video", _, socket) do
    send(self(), :start_conversion)
    {:noreply, assign(socket, loading: true, download_url: nil)}
  end

  def handle_info(:start_conversion, socket) do
    Task.start(fn ->
      Process.sleep(5000)
      Phoenix.PubSub.broadcast(Gchatdemo1.PubSub, @topic, {:video_ready, "/download/video.mp4"})
    end)

    {:noreply, socket}
  end

  def handle_info({:video_ready, url}, socket) do
    {:noreply, assign(socket, loading: false, download_url: url)}
  end
      ...
      <% else %>
        ...
        <div class="flex justify-center gap-4">
          <a
            href="/videos"
            target="_blank"
            class="px-5 py-2 bg-green-500 text-white font-semibold rounded-lg shadow-md hover:bg-green-600 transition"
          >
            Danh sách Video
          </a>
          <a
