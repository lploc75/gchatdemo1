  ...
  def start(_type, _args) do
    rtmp_server_options = %{
      ...
      handle_new_client: fn client_ref, streamer_id, stream_key ->
        streamer_id =
          case Integer.parse(streamer_id) do
            {id, ""} ->
              IO.puts("Streamer ID is valid: #{is_number(id)}")
              id

            _ ->
              IO.puts("Streamer ID is not valid")
              nil
          end

        stream_key_current =
          if is_nil(streamer_id) do
            nil
          else
        ...
        if stream_key == stream_key_current and stream_key_current != nil and !is_nil(streamer_id) and streamer_id > 0 do
