defmodule Gchatdemo1.Accounts.User do
  ...
  only: [:id, :email, :display_name, :avatar_url, :status, :last_active_at,:confirmed_at , :role]}
  schema "users" do
    field :email, :string
  ...
  defp maybe_validate_unique_email(changeset, opts) do
    if Keyword.get(opts, :validate_email, true) do
      changeset
      |> unsafe_validate_unique(:email, Gchatdemo1.Repo)
  ...
  def email_changeset(user, attrs, opts \\ []) do
    ...
    |> case do
      ...
      %{} = changeset -> add_error(changeset, :email, "Email không thay đổi")
    end
  end
  ...
  def password_changeset(user, attrs, opts \\ []) do
    user
    |> cast(attrs, [:password])
    |> validate_confirmation(:password, message: "Mật khẩu không trùng khớp")
    |> validate_password(opts)
  end
  ...
  """
  def valid_password?(%Gchatdemo1.Accounts.User{hashed_password: hashed_password}, password)
  ...
  @doc """
  Changeset cập nhật avatar. vì là link do cludinary cung cấp nên không cần validate
  """
